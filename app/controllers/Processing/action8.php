...
foreach($uniqueSeperatedKey as $val)
{
	...
	if ($type == 2)
	{
		...
		$sum_impression[$val]	= $sum_impression[$val] / 100;
		$sum_clicks[$val]		= $sum_clicks[$val] / 100;
		$sum_ctr[$val]		   = $sum_clicks[$val] / $sum_impression[$val];
		$sum_position[$val]	  = $sum_position[$val] / 100;
	}
...
if ($type == 1)
{
	...
	foreach($uniqueWords as $key => $val)
	{
		$objSheet2->getCell('B'.($m+2))->setValue($key);
		$objSheet2->getCell('C'.($m+2))->setValue($val);
		$objSheet2->getCell('D'.($m+2))->setValue(strlen($key));
		$m++;
	}
...
else if ($type == 2)
{
	...
	$objSheet2->getCell('C1')->setValue('FREQUENCY COUNT');
	$objSheet2->getCell('D1')->setValue('CHARACTER COUNT');
	$objSheet2->getCell('E1')->setValue('Summed Impressions');
	$objSheet2->getCell('F1')->setValue('Summed Clicks');
	$objSheet2->getCell('G1')->setValue('Summed CTR');
	$objSheet2->getCell('H1')->setValue('Summed Avg. position');
	...
	foreach($uniqueWords as $key => $val)
	{
		...
		$objSheet2->getCell('C'.($m+2))->setValue($val);
		$objSheet2->getCell('D'.($m+2))->setValue(strlen($key));
		$objSheet2->getCell('E'.($m+2))->setValue($sum_impression[$key]);
		$objSheet2->getCell('F'.($m+2))->setValue($sum_clicks[$key]);
		$objSheet2->getCell('G'.($m+2))->setValue($sum_ctr[$key]);
		$objSheet2->getCell('H'.($m+2))->setValue($sum_position[$key] /$val);
		$m++;
	}
...
else if ($type == 3)
{
	...
	$objSheet2->getCell('C1')->setValue('FREQUENCY COUNT');
	$objSheet2->getCell('D1')->setValue('CHARACTER COUNT');
	$objSheet2->getCell('E1')->setValue('SUMMED TRAFFIC SHARE');
	...
	foreach($uniqueWords as $key => $val)
	{
		...
		$objSheet2->getCell('C'.($m+2))->setValue($val);
		$objSheet2->getCell('D'.($m+2))->setValue(strlen($key));
		$objSheet2->getCell('E'.($m+2))->setValue($sum_traffic[$key]);
		$m++;
	}
